...
- Firebase Phone Authentication (OTP) for secure mobile verification
...
if (user role = ADMIN):
 ...
 if (loginMode = "phone"):
  ...
  → If valid: Log successful login
  → If new phone: Auto-register user
...
graph TD
    ...
    subgraph Service_Layer
        ...
        Firebase["Firebase Auth (OTP/OAuth)"]
        RateLim["Rate Limiter (In-Memory)"]
    ...
    subgraph Data_Layer
        ...
        Replica[(Future: PostgreSQL Replica)]
...
erDiagram
    ...
    USERS {
        ...
        boolean phone_verified "Not Null, Default: false"
...
## QA and Testing Milestones (April 2026)

In addition to the core architecture, we have rigorously engineered and deployed a full **Enterprise SDET Testing Suite** across both the backend and frontend to ensure all security requirements are organically upheld.

### 1. Robust Backend Security Validation (Spring Boot / JUnit 5)
- **HS512 Key Hardening:** Successfully upgraded and tested our JWT encryption generator to enforce mathematically strict 512-bit security thresholds.
- **Server Crash Prevention:** Simulated malformed/tampered JWT injections (`MalformedJwtException`) to assert that the `JwtAuthFilter` degrades queries safely to an organic HTTP `403 Forbidden` response instead of throwing catastrophic HTTP `500 Internal Server Errors`.
- **Component Isolation:** Architected strict `@WebMvcTest` environments leveraging standalone test contexts (`@AutoConfigureMockMvc(addFilters = false)`) to prove that Controller routing acts deterministically.

### 2. Frontend Lifecycle & Integration Testing (Angular 18 / Jasmine)
- **Dependency Isolation & Routing:** Decoupled Angular HTTP and Router hierarchies (`provideRouter`, `ActivatedRoute`) during UI bootstrap tests.
- **Resiliency on Status 0:** Actively simulated backend network disconnections via `HttpTestingController`, validating that our UI explicitly flags "Unable to connect..." via parsed `HttpErrorResponse` metadata.
- **Token Injection:** Spied against `jwtInterceptor` explicitly confirming that active browser sessions dynamically attach mathematical `Bearer [jwt]` tags onto subsequent outgoing HTTP packages payload headers.
- **Role-Based Redirection:** Verified that standard Form Logins securely interrogate dynamic JWT payloads, dynamically bouncing users to `['/admin']` strictly when an `ADMIN` role is detected.
